import { DuetMargin, DuetTheme } from "../../common-types";
import { ThemeableComponent } from "../../common/themeable-component";
export declare type DuetLabelSize = "small" | "medium";
export declare type DuetLabelMargin = DuetMargin | "small";
export declare class DuetLabel implements ThemeableComponent {
  /**
   * Theme of the label.
   */
  theme: DuetTheme;
  /**
   * ID of the component that the label is labelling.
   */
  for: string;
  /**
   * Controls the margin of the component.
   */
  margin: DuetLabelMargin;
  /**
   * Controls the size of the label.
   */
  size: DuetLabelSize;
  /**
   * Component lifecycle events.
   */
  componentWillLoad(): void;
  /**
   * render() function
   * Always the last one in the class.
   */
  render(): any;
}
