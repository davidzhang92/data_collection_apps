...
const DuetHeaderIcon = (props, _children) => {
  ...
  return (index.h("span", { class: `duet-header-icon ${props.class ? props.class : ""}` },
    index.h("duet-icon", { margin: "none", size: iconSize, icon: props.icon, color: "currentColor" })));
};

const DuetHeaderDropdown = (props, _children) => {
  const accessibleLabel = props.accessibleText ? props.accessibleText : props.label;
  return (index.h("div", { class: { ["duet-header-dropdown"]: true, [props.className]: !!props.className } },
    index.h("button", { ref: props.toggleRef, "aria-haspopup": "listbox", "aria-controls": props.id, "aria-expanded": props.isOpen ? "true" : "false", "aria-labelledby": `${props.id}-accessible-label`, id: `${props.id}-button`, type: "button", class: {
        [props.id]: true,
      ...
      props.icon && index.h(DuetHeaderIcon, { icon: props.icon }),
      index.h("span", { "aria-hidden": "true", id: `${props.id}-label`, class: "duet-header-dropdown-label" }, props.label),
      props.badge && index.h("div", { class: "duet-header-badge" }),
      index.h("div", { class: "duet-header-caret", "aria-hidden": "true" },
        index.h("duet-icon", { margin: "none", size: "auto", icon: actionArrowDownSmall.actionArrowDownSmall.svg, color: "currentColor" })),
      index.h("duet-visually-hidden", { id: `${props.id}-accessible-label` }, accessibleLabel)),
    index.h("div", { ref: props.itemsRef, tabindex: "-1", role: "menu", id: props.id, "aria-labelledby": `${props.id}-button`, class: {
...
const DuetHeader = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.duetChange = index.createEvent(this, "duetChange", 3);
    this.duetSessionClick = index.createEvent(this, "duetSessionClick", 7);
    this.duetSearchClick = index.createEvent(this, "duetSearchClick", 7);
    this.duetUserClick = index.createEvent(this, "duetUserClick", 7);
    this.duetLanguageSelect = index.createEvent(this, "duetLanguageSelect", 7);
    this.duetLogoClick = index.createEvent(this, "duetLogoClick", 7);
    this.duetBackClick = index.createEvent(this, "duetBackClick", 7);
    this.duetContactClick = index.createEvent(this, "duetContactClick", 7);
    ...
    this.toggleMenu = (ev, type, id, active) => {
      ...
      // For mobile menu
        ...
        if (this.isSlideOutOpen) {
          bodyScrollLock_es6.disableBodyScroll(this.navigationEl, { reserveScrollBarGap: true });
        }
        else {
          bodyScrollLock_es6.enableBodyScroll(this.navigationEl);
        }
      }
      else if (type === "sub") {
        this.isContactOpen = false;
  ...
  handleKeyDown(ev) {
    if (this.isContactOpen || this.isLanguageOpen || this.isSegmentOpen) {
      ...
      if (keyboardUtils.isSpaceKey(ev) && activeElement) {
        ev.preventDefault();
        ev.stopPropagation();
  ...
  async refresh() {
    var _a, _b;
    if (this.renderDesktopNav !== index.Build.isServer ? true : (_a = this.mql[0]) === null || _a === void 0 ? void 0 : _a.matches) {
    ...
    this.processedLanguageItems = stringUtils.parsePossibleJSON(this.languageItems);
    this.processedContactItems = stringUtils.parsePossibleJSON(this.contactItems);
    this.processedSession = stringUtils.parsePossibleJSON(this.session);
    this.processedSearch = stringUtils.parsePossibleJSON(this.search);
    this.processedUser = stringUtils.parsePossibleJSON(this.user);
  ...
  render() {
    ...
    const processedSession = () => this.processedSession ? (index.h("a", { href: this.processedSession.href || "#", id: this.processedSession.id, class: "duet-header-logout duet-header-button", onClick: this.handleSessionClick }, index.h(DuetHeaderIcon, { class: "desktop", icon: this.processedSession.type === "logout" ? navigationLogout.svg : navigationLogin.svg, iconSize: "x-small" }), index.h(DuetHeaderIcon, { class: "mobile", icon: this.processedSession.type === "logout" ? navigationLogout.svg : navigationLogin.svg, iconSize: "small" }), index.h("span", { class: "duet-header-button-label" }, this.processedSession.label))) : null;
    const processedSessionDropdown = () => this.processedSession && this.processedSession.sessionItems ? (index.h(DuetHeaderDropdown, { className: "duet-header-session", id: "duet-header-session", label: this.processedSession.label, icon: this.processedSession.type === "logout" ? navigationLogout.svg : navigationLogin.svg, active: this.isSessionOpen, isOpen: this.isSessionOpen, badge: false, toggleRef: el => (this.sessionItemsButtonEl = el), itemsRef: el => (this.sessionItemsEl = el), onToggle: event => this.toggleMenu(event, "session") }, this.processedSession.sessionItems.map((sessionItem, index$1) => (index.h(index.Fragment, null, sessionItem.label && index.h("span", { class: "duet-header-session-label" }, sessionItem.label), index.h("duet-spacer", { size: "small", "aria-hidden": "true" }), sessionItem.items.map(item => (index.h("a", { role: "menuitem", "aria-label": sessionItem.label ? `${sessionItem.label} ${item.label}` : item.label, id: item.id, href: item.href || "#", onClick: event => this.handleSessionItemClick(event, item), onKeyDown: this.handleArrowKeys }, item.label))), index$1 !== this.processedSession.sessionItems.length - 1 && (index.h(index.Fragment, null, index.h("duet-spacer", { size: "x-small", "aria-hidden": "true" }), index.h("duet-divider", { margin: "none", "aria-hidden": "true" })))))))) : null;
    const processedSearch = () => this.processedSearch ? (index.h("a", { href: this.processedSearch.href || "#", id: this.processedSearch.id, class: "duet-header-search duet-header-button", onClick: this.handleSearchClick }, index.h(DuetHeaderIcon, { icon: formSearch.formSearch.svg }), index.h("span", { class: "duet-header-button-label" }, this.processedSearch.label))) : null;
    // Back variation
    if (this.processedBack) {
      return (index.h(index.Host, { class: "duet-header-back-variation" }, index.h("div", { class: "duet-header-reserved-space" }, index.h("header", { class: {
          "duet-header": true,
    ...
    const defaultVariationContent = (index.h("div", { class: {
      ...
      } }, index.h("header", { class: {
        "duet-header": true,
        "duet-theme-turva": this.theme === "turva",
        "duet-header-inactive": !this.processedItems,
      }, part: "header" }, index.h("div", { class: "duet-header-top" }, skipToLabel(), logo(), index.h("slot", { name: "logo-utility" }, this.region && index.h("div", { class: "duet-header-region" }, this.region)), (this.processedItems ||
      languageDropdown() ||
      contactDropdown() ||
      this.processedSession ||
      this.processedUser) && (index.h("button", { class: { "duet-header-toggle": true, active: this.isSlideOutOpen }, onClick: event => this.toggleMenu(event, "mobile"), type: "button" }, index.h("duet-visually-hidden", null, this.accessibleLabel), index.h(DuetHeaderHamburger, null)))), index.h("div", { ref: el => (this.navigationEl = el), class: {
        "duet-header-bottom": true,
        active: this.isSlideOutOpen,
        inactive: !this.processedItems,
      } }, index.h("nav", { class: "duet-header-items", role: "navigation" }, this.processedItems &&
      this.processedItems.map((item, itemIndex) => (index.h("div", { class: "duet-header-item-container" }, index.h("a", { class: {
    ...
    if (itemsHaveSubItems) {
      ...
      return (index.h(index.Host, { class: "duet-header-multi-level-variation" }, index.h("div", { class: {
...
DuetHeader.style = duetHeaderCss;
...
const DefaultLogo = ({ inverse, logoId, language }) => {
  const logoColor = inverse ? tokens_module.colorGrayLightest : tokens_module.colorBrandBlue;
  const { title, path, viewBox } = i18n[language];
  return (index.h("svg", { role: "img", xmlns: "http://www.w3.org/2000/svg", fill: logoColor, "aria-labelledby": logoId, viewBox: viewBox }, index.h("title", { id: logoId }, title), index.h("path", { d: path })));
};
const DuetLogo = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    /**
     * Own Properties.
     */
    this.logoId = createId.createID("DuetLogo");
    /**
     * Theme of the logo.
     */
    this.theme = "";
  ...
  render() {
    ...
    return (index.h(index.Host, { class: { "duet-m-0": this.margin === "none" } }, index.h(TagName, { href: this.href, ref: link => (this.nativeLink = link), class: {
        "duet-logo": true,
        [this.size]: true,
        "duet-theme-turva": this.theme === "turva",
        "duet-logo-en": this.language === "en",
        "duet-logo-sv": this.language === "sv",
      } }, this.theme === "turva" ? (index.h(TurvaLogo, { logoId: this.logoId, inverse: this.inverse })) : (index.h(DefaultLogo, { logoId: this.logoId, inverse: this.inverse, language: this.language })))));
  }
  get element() { return index.getElement(this); }
};
DuetLogo.style = duetLogoCss;

exports.duet_header = DuetHeader;
exports.duet_logo = DuetLogo;
