...
const DatePickerDay = ({ focusedDay, today, day, language, onDaySelect, onKeyboardNavigation, ref, inRange, isSelected, }) => {
  ...
  return (index.h("button", { class: { "duet-date-table-button": true, outside: isOutsideRange, disabled: isDisabled, today: isToday }, tabIndex: isFocused ? 0 : -1, onClick: handleClick, onKeyDown: onKeyboardNavigation, "aria-disabled": isOutsideRange || isDisabled, "aria-pressed": isSelected ? "true" : "false", type: "button", ref: ref },
    index.h("span", { "aria-hidden": "true" }, day.getDate()),
    index.h("duet-visually-hidden", null, formatterShort.format(day))));
};
...
const DatePickerMonth = ({ selectedDate, focusedDate, labelledById, theme, language, min, max, onDateSelect, onKeyboardNavigation, focusedDayRef, onFocusEnter, onFocusExit, }) => {
  ...
  return (index.h("table", { class: { "duet-date-table": true, "duet-theme-turva": theme === "turva" }, "aria-labelledby": labelledById, 
    // @ts-ignore
    onFocusin: onFocusEnter, onFocusout: handleFocusOut },
    index.h("thead", null,
      index.h("tr", null, dayLabels.map(label => (index.h("th", { scope: "col" },
        index.h("span", { "aria-hidden": "true" }, label.substr(0, 2)),
        index.h("duet-visually-hidden", null, label)))))),
    index.h("tbody", null, chunk(days, 7).map(week => (index.h("tr", { class: "duet-date-table-row" }, week.map(day => (index.h("td", { class: "duet-date-table-cell" },
      index.h(DatePickerDay, { day: day, today: today, language: language, focusedDay: focusedDate, inRange: inRange(day, min, max), isSelected: isEqual(day, selectedDate), onDaySelect: onDateSelect, onKeyboardNavigation: onKeyboardNavigation, ref: el => {
...
const DuetDatePicker = class {
  constructor(hostRef) {
    ...
    this.monthSelectId = createId.createID("DuetDatePicker");
    this.yearSelectId = createId.createID("DuetDatePicker");
    this.dialogLabelId = createId.createID("DuetDatePicker");
     ...
     * @default { fi: "pp.kk.vvvv", en: "dd.mm.yyyy", sv: "dd.mm.åååå" }
     */
    this.placeholder = languageUtils.getLocaleString(this.placeholderDefaults, this.language);
    /**
  ...
  render() {
    ...
    return (index.h(index.Host, { class: { "duet-expand": this.expand, "duet-m-0": this.margin === "none" } }, index.h("div", { class: "duet-date" }, index.h(DatePickerInput, { value: this.value, valueAsDate: valueAsDate, formattedValue: this.inputValue, language: this.language, onInput: this.handleInputChange, onBlur: this.handleBlur, onFocus: this.handleFocus, onClick: this.toggleOpen, name: this.name, disabled: this.disabled, error: this.error, role: this.role, required: this.required, theme: this.theme, tooltip: this.tooltip, tooltipDirection: this.tooltipDirection, labelHidden: this.labelHidden, placeholder: this.placeholder, inputLabel: this.label, identifier: this.identifier, buttonRef: element => (this.datePickerButton = element), inputRef: element => (this.datePickerInput = element), accessibleActiveDescendant: this.accessibleActiveDescendant, accessibleControls: this.accessibleControls, accessibleOwns: this.accessibleOwns, accessibleDescribedBy: this.accessibleDescribedBy, echoPlaceholder: this.echoPlaceholder, caption: this.caption })), index.h("div", { class: {
        "duet-theme-turva": this.theme === "turva",
        "duet-date-dialog": true,
        left: this.direction === "left",
        error: !!this.error,
        active: this.open,
      }, role: "dialog", "aria-modal": "true", "aria-hidden": this.open ? "false" : "true", "aria-labelledby": this.dialogLabelId, onTouchMove: this.handleTouchMove, onTouchStart: this.handleTouchStart, onTouchEnd: this.handleTouchEnd }, index.h(focusUtils.FocusGuard, { moveFocusTo: this.focusedDayNode }), index.h("div", { class: "duet-date-dialog-wrapper", onKeyDown: this.handleEscKey, ref: element => (this.dialogWrapperNode = element) }, index.h("duet-visually-hidden", { "aria-live": "polite" }, text.keyboardInstruction), index.h("div", { class: "duet-date-dialog-mobile-header" }, index.h("duet-label", { margin: "none" }, this.label), index.h("button", { class: "duet-date-picker-close", ref: element => (this.closeButtonNode = element), onClick: () => this.hide(), type: "button" }, index.h("duet-icon", { name: "navigation-close-small", margin: "none", color: "currentColor", size: "xx-small" }), index.h("duet-visually-hidden", null, text.closeLabel))), index.h("div", { class: "duet-date-dialog-header" }, index.h("div", { class: "duet-date-dialog-dropdowns" }, index.h("duet-visually-hidden", null, index.h("h2", { id: this.dialogLabelId, "aria-live": "polite", "aria-atomic": "true" }, text.monthLabels[focusedMonth], " ", this.focusedDay.getFullYear())), index.h("duet-visually-hidden", null, index.h("label", { htmlFor: this.monthSelectId }, text.monthSelectLabel)), index.h("div", { class: "duet-date-dialog-select" }, index.h("select", { id: this.monthSelectId, class: "duet-date-month-select", ref: element => (this.monthSelectNode = element), onChange: this.handleMonthSelect }, text.monthLabels.map((month, i) => (index.h("option", { key: month, value: i, selected: i === focusedMonth, disabled: !inRange(new Date(focusedYear, i, 1), minDate ? startOfMonth(minDate) : null, maxDate ? endOfMonth(maxDate) : null) }, month)))), index.h("div", { class: "duet-date-dialog-select-label", "aria-hidden": "true" }, index.h("span", null, text.monthLabelsShort[focusedMonth]), index.h("duet-icon", { name: "action-arrow-down-small", color: "currentColor", margin: "none", size: "xxx-small" }))), index.h("duet-visually-hidden", null, index.h("label", { htmlFor: this.yearSelectId }, text.yearSelectLabel)), index.h("div", { class: "duet-date-dialog-select" }, index.h("select", { id: this.yearSelectId, class: "duet-date-year-select", onChange: this.handleYearSelect }, range(minYear, maxYear).map(year => (index.h("option", { key: year, selected: year === focusedYear }, year)))), index.h("div", { class: "duet-date-dialog-select-label", "aria-hidden": "true" }, index.h("span", null, this.focusedDay.getFullYear()), index.h("duet-icon", { name: "action-arrow-down-small", color: "currentColor", margin: "none", size: "xxx-small" })))), index.h("div", { class: "duet-date-dialog-buttons" }, index.h("duet-action-button", { class: "duet-date-dialog-prev", onClick: this.handlePreviousMonthClick, disabled: prevMonthDisabled, "icon-name": "action-arrow-left-small" }, text.prevMonthLabel), index.h("duet-action-button", { class: "duet-date-dialog-next", onClick: this.handleNextMonthClick, disabled: nextMonthDisabled, "icon-name": "action-arrow-right-small" }, text.nextMonthLabel))), index.h(DatePickerMonth, { selectedDate: valueAsDate, focusedDate: this.focusedDay, onDateSelect: this.handleDaySelect, onKeyboardNavigation: this.handleKeyboardNavigation, labelledById: this.dialogLabelId, language: this.language, theme: this.theme, min: minDate, max: maxDate, focusedDayRef: element => (this.focusedDayNode = element), onFocusEnter: this.enableActiveFocus, onFocusExit: this.disableActiveFocus })), index.h(focusUtils.FocusGuard, { moveFocusTo: this.closeButtonNode }))));
  }
