USE [DataCollection]
GO
/****** Object:  StoredProcedure [dbo].[InsertToVwResultEntry]    Script Date: 29-Apr-24 2:50:42 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[InsertToVwResultEntry]
AS
BEGIN
    ...
    SELECT 
        ...
        CASE 
            WHEN pr.id IS NOT NULL THEN 'programming'
            WHEN lr.id IS NOT NULL THEN 'leaktest'
			WHEN er.id IS NOT NULL THEN 'endtest'
            WHEN lsr.id IS NOT NULL THEN 'laser'
            WHEN oqcr.id IS NOT NULL THEN 'oqc'
            ELSE 'unknown'
        END,
    ...
    LEFT JOIN endtest_result_entry er ON vre.record_id = er.id AND er.part_id is null
	...
	WHERE id IN (SELECT a.id FROM vw_result_entry  a INNER JOIN part_master b ON a.part_id = b.id WHERE b.is_deleted = 1)
END