...
@app.route('/api/get_pagination_laser_result_entry_count', methods=['GET'])
def get_pagination_laser_result_entry_count():
    cursor = conn.cursor()
    
    # Construct the SQL query to select all data from the part_master table
    query = "select count(id) from laser_result_entry where is_deleted = 0"
