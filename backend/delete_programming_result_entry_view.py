...
@app.route('/api/delete_programming_result_entry_view', methods=['GET'])
@token_required
def delete_programming_result_entry_view():
    try:
        ...
        if ids:
            ...
            query = f"UPDATE  programming_result_entry  SET deleted_date = getdate(), is_deleted = 1, deleted_by = ? WHERE id IN ({placeholders})"
            cursor.execute(query, (user_id,) + tuple(ids))
        elif id_single:
            # Single record delete
            query = "UPDATE  programming_result_entry  SET deleted_date = getdate(), is_deleted = 1, deleted_by = ? WHERE id = ?"
