...
def delete_leaktest_result_entry_view():
    try:
        ...
        if ids:
            ...
            query = f"UPDATE  leaktest_result_entry  SET deleted_date = getdate(), is_deleted = 1 WHERE id IN ({placeholders})"
            cursor.execute(query, tuple(ids))
        elif id_single:
            # Single record delete
            query = "UPDATE  leaktest_result_entry  SET deleted_date = getdate(), is_deleted = 1 WHERE id = ?"
