...
# --------------------
@app.route('/api/get_laser_result_entry_view', methods=['GET'])
def get_laser_result_entry_view():
    try:
        ...
        query = "select a.id AS id, b.part_no, a.serial_no, a.data_matrix, a.label_id, a.result, c.defect_description, a.created_date from laser_result_entry a inner join part_master b on a.part_id = b.id left join defect_master c on a.defect_id = c.id where a.is_deleted='0' order by created_date desc OFFSET ? ROWS FETCH NEXT 10 ROWS ONLY"
...
@app.route('/api/get_filter_search_laser_result_entry_view', methods=['GET'])
def get_filter_search_laser_result_entry_view():
    ...
    query = "select a.id AS id, b.part_no, a.serial_no, a.data_matrix, a.label_id, a.result, c.defect_description, a.created_date from laser_result_entry a inner join part_master b on a.part_id = b.id left join defect_master c on a.defect_id = c.id WHERE 1=1"
...
# --------------------
@app.route('/api/get_laser_result_report', methods=['POST'])
def get_laser_result_report():
    # Define the path to the Excel template (modify this path accordingly)
    template_path = r'/data-storage/sfdc_apps/excel_import/laser_report_template.xlsx'
    ...
    try:
        ...
        return Response(
            ...
            headers={
                'Content-Disposition': 'attachment; filename=laser_report.xlsx'
            }
        )
