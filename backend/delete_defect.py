...
def delete_defect():
    try:
        ...
        if ids:
            ...
            query = f"UPDATE defect_master SET deleted_date = getdate(), is_deleted = 1 WHERE id IN ({placeholders})"
            cursor.execute(query, tuple(ids))
        elif id_single:
            # Single record delete
            query = "UPDATE defect_master SET deleted_date = getdate(), is_deleted = 1 WHERE id = ?"
