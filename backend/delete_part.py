...
def delete_part():
    try:
        ...
        if ids:
            ...
            query = f"UPDATE part_master SET deleted_date = getdate(), deleted_by = ?, is_deleted = 1 WHERE id IN ({placeholders})"
            cursor.execute(query, (user_id,) + tuple(ids))
        elif id_single:
            # Single record delete
            query = "UPDATE part_master SET deleted_date = getdate(), deleted_by = ?, is_deleted = 1 WHERE id = ?"
