...
@app.route('/api/get_pagination_user_entries', methods=['GET'])
def get_pagination_user_entries():
    cursor = conn.cursor()
    
    # Construct the SQL query to select all data from the part_master table
    query = "select count(id) from user_master where is_deleted = 0 and  is_superadmin = 0"
