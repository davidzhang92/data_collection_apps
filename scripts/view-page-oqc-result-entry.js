...
function renderData(data) {
	...
	// Function to GET data from the backend API for pagination
		...
		function fetchData(pageId) {
			const apiEndpoint = filteredData.length > 0 ?
				'' :
				'http://localhost:4000/api/oqc_result_entry_view_api';
		...
		$('#search-part').click(function () {
			...
			if (partNumber || dateFrom || dateTo) {
				...
				$.ajax({
					url: 'http://localhost:4000/api/filter_search_oqc_result_entry_view_api',
					type: 'GET',
	...
	// Handle delete and DELETE request
		...
		$('#submit-data-delete').on('click', function(event) {
			...
			$.ajax({
				url: 'http://localhost:4000/api/delete_oqc_result_entry_view_api',
				type: 'DELETE',
				data: JSON.stringify({
					id: deleteCurrentId,
				}),
	...
	function fetchPaginationEntriesCount() {
		$.ajax({
			type: 'GET',
			url: 'http://localhost:4000/api/pagination_oqc_result_entry_count_api',
			dataType: 'json',
			success: function (response) {
