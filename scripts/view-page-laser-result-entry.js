...
function renderData(data) {
	...
	// Function to GET data from the backend API for pagination
		...
		function fetchData(pageId) {
			const apiEndpoint = filteredData.length > 0 ?
				'' :
				'http://' + window.location.hostname + ':4000/api/laser_result_entry_view_api';
	...
	function fetchPaginationEntriesCount() {
		$.ajax({
			type: 'GET',
			url: 'http://' + window.location.hostname + ':4000/api/pagination_laser_result_entry_count_api',
			dataType: 'json',
			success: function (response) {
